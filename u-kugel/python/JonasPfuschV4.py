...
y_50_norm = y_50 - y_dark  - np.mean(y_50-y_dark)
...
Y_m_v_bright = np.fft.fft(y_50_norm, axis=1) / np.sqrt(w)
...
plt.hlines(y=PRNU*100, xmin=0, xmax=1, color='black', linestyles = '--',label='PRNU')
plt.hlines(y=sigma_y_2_stack_bright, xmin=0, xmax=1, color='black', linestyles='-.', label='sigma_y_2_stack_bright')
plt.legend()
plt.show()
#plt.savefig("Spektogram_Horizontal_PRNU.png",dpi=600)

'Vertikales Spektroggramm DSNU'
Y_n_v_dark = np.fft.fft(y_dark_norm, axis=0) /np.sqrt(h)
...
Y_n_v_bright = np.fft.fft(y_50_norm, axis=0) / np.sqrt(h)
...
plt.legend()
plt.show()
#plt.savefig("Spektogram_Vertikal_PRNU.png",dpi=600)
